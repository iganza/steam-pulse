"""Unit tests for NewReleasesService — window math, bucketing, paging clamps."""
...
def test_window_start_date_today_is_today() -> None:
    today = date(2026, 4, 8)
...
def test_get_released_returns_envelope_and_counts() -> None:
    ...
    assert set(result["counts"].keys()) == {"today", "week", "month", "all"}
    assert result["filters"] == {"genre": None, "tag": None}
    # Headline counts call the repo for each window + total = 5
...
def test_get_added_translates_window_to_datetime() -> None:
    ...
    # First positional arg is the `since` datetime — must be tz-aware UTC.
...
def test_window_all_returns_none_lower_bound() -> None:
    assert _window_start("all") is None
    assert _window_start_date("all") is None


def test_get_released_all_window_passes_none_to_repo() -> None:
    repo = MagicMock()
    repo.find_recently_released.return_value = []
    repo.count_released_between.return_value = 0
    svc = NewReleasesService(repo)

    svc.get_released("all", page=1, page_size=24)

    args, _ = repo.find_recently_released.call_args
    assert args[0] is None  # since=None means "no lower bound"
