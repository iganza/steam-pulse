...
1. **CDK diff** — `cd infra && npx cdk diff SteamPulseDeliveryStack`. Should
   show one new `AWS::CloudFront::Function` resource and FunctionAssociations
   added on the default behavior plus each additional behavior (≥5 total —
   default, the shared `api_cached_behavior` covering 4 SSR-fanout paths,
   `/api/*`, `/_next/static/*`, `/static/*`). No distribution recreation.
2. **Deploy**, then:
   ```bash
