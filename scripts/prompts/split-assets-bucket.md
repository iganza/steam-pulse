...
- The constructor already receives `assets_bucket` (from `DataStack`). Add a `frontend_bucket` parameter alongside it.
- Change `CACHE_BUCKET_NAME` env var on the frontend Lambda to point at `steampulse-frontend-{env}` (or pass the bucket name via the new `frontend_bucket` prop).
