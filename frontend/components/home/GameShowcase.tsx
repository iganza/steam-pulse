...
function ShowcaseContent({ game }: { game: ShowcaseGame }) {
  ...
  const hasCharts = game.timeline.length >= 2 || topOverlaps.length > 0;

  return (
    <div className={hasCharts ? "grid md:grid-cols-2 gap-0" : ""}>
      ...
      {/* Right — charts (hidden when no data) */}
      {(game.timeline.length >= 2 || topOverlaps.length > 0) && (
      <div
        className="p-6 flex flex-col gap-6"
        style={{ borderLeft: "1px solid var(--border)" }}
      >
        {game.timeline.length >= 2 && <SentimentTimeline timeline={game.timeline} />}
