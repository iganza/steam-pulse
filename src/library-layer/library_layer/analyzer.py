...
from library_layer.llm.backend import LLMBackend, LLMRequest, LLMUsage
...
def parse_chunk_record_id(record_id: str) -> tuple[int, int, int, str] | None:
    """Recover (appid, chunk_index, review_count, chunk_hash) from a chunk record_id.
...
def run_chunk_phase(
    *,
    ...
    def _persist_fresh(idx: int, response: BaseModel, usage: LLMUsage) -> None:
...
def run_synthesis_phase(
    *,
    ...
    if merged.total_stats.date_range_start and merged.total_stats.date_range_end:
