...
const isIgnoredFailedResource = (value: string) => {
...
export function collectConsoleErrors(page: Page) {
  ...
  page.on('console', (msg) => {
    if (msg.type() === 'error') {
      const text = msg.text()
      // Only ignore known-benign resource failures (e.g. favicon)
      if (text.includes('Failed to load resource') && isIgnoredFailedResource(text)) return
  ...
  page.on('requestfailed', (request) => {
    ...
    if (isIgnoredFailedResource(url)) return
    const failureText = request.failure()?.errorText ?? 'unknown error'
    // Aborted requests are normal during navigation (in-flight fetches cancelled)
    if (failureText.includes('net::ERR_ABORTED')) return
    errors.push(`[requestfailed] ${request.method()} ${url} - ${failureText}`)
  })

  page.on('pageerror', (err) => {
    // Hydration mismatches in dev mode are noisy but not actionable
    if (err.message.includes('Hydration')) return
    // Minified React errors are real crashes in production but noise in dev
    if (isLocalDev && err.message.includes('Minified React error')) return
...
/** Assert the page loaded without errors and has content. */
export async function assertPageLoadsOk(page: Page) {
  ...
  // Detect Next.js error pages (404, 500, runtime errors)
  const bodyText = await page.locator('body').innerText()
  expect(bodyText).not.toContain('Application error: a client-side exception')
  expect(bodyText).not.toContain('Internal Server Error')
  // Page title should be non-empty and not a generic error
  const title = await page.title()
  expect(title.length).toBeGreaterThan(0)
  expect(title).not.toMatch(/^500:|^404:/)
}
