...
test.describe('Genre synthesis page — pre-order report', () => {
  test('buy block renders in pre-order state with ship date', async ({ page }) => {
    ...
    await expect(block).toContainText(/Pre-order\s+—\s+\$49/)
...
test.describe('Genre synthesis page — live report', () => {
  test('buy block renders in live state with Buy buttons', async ({ page }) => {
    ...
    await expect(block).toContainText(/Buy\s+—\s+\$49/)
    await expect(block).toContainText(/available now/i)
  })
})

test.describe('Genre synthesis page — no synthesis row', () => {
  test('unknown slug redirects to /search filtered by genre', async ({ page }) => {
    // Pre-launch, only some genres have a synthesis row. Clicking a Browse
    // link for an un-synthesised genre lands on the search listing rather
    // than a 404 — preserves user intent (browse games in this genre).
    await page.goto('/genre/rdb-missing')
    await page.waitForURL(/\/search\?genre=rdb-missing/)
    expect(page.url()).toContain('/search?genre=rdb-missing')
  })
})
