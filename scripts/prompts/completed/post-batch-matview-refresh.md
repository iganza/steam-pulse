...
Reuse `dispatch_batch.py` with an `"action": "post_batch"` branch. A Pass state before
the DistributedMap computes `appids_count` via `States.ArrayLength`; the map discards
per-item results (`result_path: DISCARD`) to stay under the 256KB state limit. The
post-batch Lambda receives `execution_id` and `appids_count`, and publishes
`BatchAnalysisCompleteEvent` to system-events-topic.
...
The task has retry (2 attempts) and catch → Succeed so a transient publish failure
doesn't fail the orchestrator.
