...
test.describe('/new-releases — three-lens feed', () => {
  ...
  test('All time window pill is present and selectable', async ({ page }) => {
    await mockAllApiRoutes(page)
    await page.goto('/new-releases')
    await expect(page.getByTestId('window-all')).toBeVisible()
    await page.getByTestId('window-all').click()
    await expect(page).toHaveURL(/window=all/)
  })

  test('genre filter dropdown updates URL and shows chip', async ({ page }) => {
    await mockAllApiRoutes(page)
    await page.goto('/new-releases')
