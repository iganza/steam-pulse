...
--
-- Each matview expands the filtered base game set across the four values in
-- the `grains` CTE (week, month, quarter, year) via CROSS JOIN, so every game
-- participates in four granularity-aware DATE_TRUNC aggregations in a single
-- pass. Games are filtered to review_count >= 10 and type='game'. The
-- ea_flags CTE is folded in so EA metrics sit directly in the matview.
--
