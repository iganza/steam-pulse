...
MATERIALIZED_VIEWS: tuple[str, ...] = (
    ...
    # 0047_discovery_feeds_matview — homepage discovery rows (popular / top_rated /
    ...
    -- NOT MATERIALIZED so Postgres inlines `base` and per-feed CTEs can use
    -- existing games(review_count / hidden_gem_score / last_analyzed) btree
    -- indexes. `top_rated` (positive_pct) and `new_release` (release_date)
    -- have no matching index and fall back to a seq-scan + top-N heap sort
    -- at refresh time — acceptable at the 5min/6h refresh cadence.
    ...
    popular AS (
        SELECT 'popular'::text AS feed_kind,
               ...
               base.*
        FROM base WHERE review_count IS NOT NULL
        ORDER BY review_count DESC NULLS LAST, appid LIMIT 24
    ),
    top_rated AS (
        SELECT 'top_rated'::text AS feed_kind,
               ...
               base.*
        FROM base WHERE review_count >= 200 AND positive_pct IS NOT NULL
        ORDER BY positive_pct DESC NULLS LAST, review_count DESC NULLS LAST, appid LIMIT 24
    ),
    hidden_gem AS (
        SELECT 'hidden_gem'::text AS feed_kind,
               ROW_NUMBER() OVER (ORDER BY hidden_gem_score DESC NULLS LAST, appid) AS rank,
               base.*
        FROM base WHERE hidden_gem_score IS NOT NULL
        ORDER BY hidden_gem_score DESC NULLS LAST, appid LIMIT 24
    ),
    new_release AS (
        SELECT 'new_release'::text AS feed_kind,
               ROW_NUMBER() OVER (ORDER BY release_date DESC NULLS LAST, appid) AS rank,
               base.*
        FROM base WHERE release_date IS NOT NULL AND COALESCE(coming_soon, FALSE) = FALSE
        ORDER BY release_date DESC NULLS LAST, appid LIMIT 24
    ),
    just_analyzed AS (
        SELECT 'just_analyzed'::text AS feed_kind,
               ROW_NUMBER() OVER (ORDER BY last_analyzed DESC NULLS LAST, appid) AS rank,
               base.*
        FROM base WHERE last_analyzed IS NOT NULL
        ORDER BY last_analyzed DESC NULLS LAST, appid LIMIT 24
    )
    ...
    SELECT feed_kind, rank, appid, name, slug, developer, header_image,
           ...
           hidden_gem_score, last_analyzed,
           estimated_owners, estimated_revenue_usd, revenue_estimate_method,
           is_early_access FROM top_rated
    UNION ALL
    SELECT feed_kind, rank, appid, name, slug, developer, header_image,
           review_count, review_count_english, positive_pct, review_score_desc,
           price_usd, is_free, release_date, deck_compatibility,
           hidden_gem_score, last_analyzed,
           estimated_owners, estimated_revenue_usd, revenue_estimate_method,
           is_early_access FROM hidden_gem
    UNION ALL
    SELECT feed_kind, rank, appid, name, slug, developer, header_image,
           review_count, review_count_english, positive_pct, review_score_desc,
           price_usd, is_free, release_date, deck_compatibility,
           hidden_gem_score, last_analyzed,
           estimated_owners, estimated_revenue_usd, revenue_estimate_method,
           is_early_access FROM new_release
    UNION ALL
    SELECT feed_kind, rank, appid, name, slug, developer, header_image,
           review_count, review_count_english, positive_pct, review_score_desc,
           price_usd, is_free, release_date, deck_compatibility,
...
def create_matviews(conn: object) -> None:
    ...
    with conn.cursor() as cur:  # type: ignore[union-attr]
        ...
        for view in (
            ...
            "mv_trend_by_genre",
            "mv_trend_by_tag",
            # 0047_discovery_feeds_matview — drop so persistent test DBs pick up
            # any future shape changes (new feed_kind, new column, etc.).
            "mv_discovery_feeds",
        ):
