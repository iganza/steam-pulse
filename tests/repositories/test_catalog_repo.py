...
def test_find_due_meta_includes_tier_c_if_coming_soon(
    catalog_repo: CatalogRepository,
) -> None:
    """coming_soon=TRUE bypasses Tier C — unreleased games get A-tier refresh."""
    catalog_repo.bulk_upsert([{"appid": 7710, "name": "Unreleased"}])
    catalog_repo.set_meta_status(7710, "done")
    _seed_game_row(catalog_repo, 7710, coming_soon=True, review_count=0)
    _set_meta_crawled_at(catalog_repo, 7710, days_ago=20)

    appids = [e.appid for e in catalog_repo.find_due_meta(limit=10, config=_config())]
    assert 7710 in appids


def test_find_due_meta_includes_tier_c_if_ea_genre(
    catalog_repo: CatalogRepository,
) -> None:
    """Early Access (genre 70) bypasses Tier C — EA games get A-tier refresh."""
    catalog_repo.bulk_upsert([{"appid": 7720, "name": "EA Long Tail"}])
    catalog_repo.set_meta_status(7720, "done")
    _seed_game_row(catalog_repo, 7720, review_count=2)
    _attach_genre(catalog_repo, 7720, 70)
    _set_meta_crawled_at(catalog_repo, 7720, days_ago=20)

    appids = [e.appid for e in catalog_repo.find_due_meta(limit=10, config=_config())]
    assert 7720 in appids
