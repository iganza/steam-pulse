...
> **Status:** engineering roadmap, not a packaging spec. Tier 1/2/3 below are engineering priority phases (highest-value-fastest / more-engineering / needs-new-infrastructure), not pricing tiers. Monetization direction lives in `scripts/prompts/monetization-direction.md`; this file is data-only.
...
5. [Feature catalog — priority-ordered](#feature-catalog)
   - [Tier 1 — Highest priority, ships fastest (1–10)](#tier-1)
   - [Tier 2 — High priority, more engineering (11–23)](#tier-2)
   - [Tier 3 — Lower priority, needs new infrastructure or parsing (24–31)](#tier-3)
6. [What feeds the per-game report](#pdf-reports)
...
```
   ...
   └─────────────────────────────────────────────────────────────────┘
                    ...
                    Both feed: web dashboards, API endpoints, per-game reports.
```
...
Each feature includes: type tag, question answered, data, implementation outline, competitor gap, audience value (Dev / Marketer / Studio / Publisher / Owner / Consumer).

<a id="tier-1"></a>

### Tier 1 — Highest engineering priority, ships fastest (#1–10)
...
### Tier 2 — High engineering priority, more work (#11–23)
...
### Tier 3 — Lower priority, needs new infrastructure or parsing (#24–31)
...
## What feeds the per-game report

Both layers feed into the per-game report. The per-genre table below is retained as engineering inventory; whether per-genre artifacts ever ship is a separate product question.

### Per-game report (companion to LLM `GameReport`)
...
### Per-genre artifact (companion to `GenreSynthesis`) — engineering inventory, not a committed deliverable
