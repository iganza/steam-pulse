# Fix frontend/backend metadata desyncs (Part 2: audit follow-ups)
...
English-implicit sentiment metrics. Those three are already fixed —
see Part 1 below for reviewer context.

A follow-up audit of the same area (handler.py + the game page
resolver + every other per-entity page) turned up three more
instances of the same pattern. This prompt covers the follow-up PR.
The three follow-up fixes share root cause with the already-shipped
three; grouping them into one PR lets the reviewer see the full
picture.
...
## Part 1 — What already shipped (reviewer context, do NOT re-do)

These three landed on `main` after PR #97 was reverted. Summarised
here so the reviewer of *this* PR can see the full arc. No code
changes needed for Part 1 — it's just orientation.
...
**Shipped fix:** added `"name": game.name`, `"slug": game.slug`, and
...
**Shipped fix:** once `g.name` is present on `game_meta` (1.1), set
...
**Shipped fix:** the backend now exposes `review_count_english` on
