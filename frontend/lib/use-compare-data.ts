...
/** Read-only accessor so other surfaces (e.g. GamePicker) can avoid duplicate
 *  /report fetches for appids useCompareData has already resolved. */
export function getCachedGameMeta(appid: number): CompareGameMeta | undefined {
  return gameMetaCache.get(appid);
}
