...
import { parseLocalDate, relativeTime as sharedRelativeTime, slugify } from "@/lib/format";
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 text-xs font-mono text-muted-foreground">
          ...
          {lens === "upcoming" && entry.release_date && (
            // parseLocalDate avoids the UTC-midnight → west-of-UTC day shift
            // that `new Date("YYYY-MM-DD")` causes.
            <span>{parseLocalDate(entry.release_date).toLocaleDateString()}</span>
          )}
