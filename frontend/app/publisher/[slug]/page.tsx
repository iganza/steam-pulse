...
import { getGames, getPublisherAnalytics } from "@/lib/api";
import { GameCard } from "@/components/game/GameCard";
import { Breadcrumbs } from "@/components/layout/Breadcrumbs";
import { PublisherPortfolio } from "@/components/analytics/PublisherPortfolio";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await getGames({ publisher: slug, limit: 1 });
    canonical = res.games.find((g) => g.publisher_slug === slug)?.publisher;
  } catch {
  ...
  return {
    title: `${name} \u2014 Publisher Profile`,
    description: `All Steam games published by ${name} \u2014 player sentiment analysis across their catalog.`,
    openGraph: {
      title: `${name} — Publisher Profile — SteamPulse`,
      description: `All Steam games published by ${name} — player sentiment analysis across their catalog.`,
      url: `https://steampulse.io/publisher/${slug}`,
    ...
    alternates: { canonical: `https://steampulse.io/publisher/${slug}` },
  };
}
...
export default async function PublisherPage({ params }: Props) {
  ...
  // Fetch games and publisher analytics in parallel
  const [gamesResult, portfolioResult] = await Promise.allSettled([
    getGames({ publisher: slug, sort: "review_count", limit: 100 }),
    getPublisherAnalytics(slug),
  ]);
  ...
  const canonical = games.find((g) => g.publisher_slug === slug)?.publisher;
  const name =
    canonical ?? slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        {portfolio && portfolio.games.length > 0 && (
          <section className="mb-10 space-y-4">
            <h2 className="font-serif text-lg font-semibold">Publisher Analytics</h2>
            <PublisherPortfolio data={portfolio} />
          </section>
        )}
