...
export const COMPARE_METRICS: MetricRow[] = [
  ...
  },
  {
    ...
    label: "Genre Sentiment Percentile",
    group: "audience",
    direction: "higher",
    free: false,
    // `benchmarks.sentiment_rank` is already a 0..1 percentile computed via
    // PERCENT_RANK() in the backend, where higher values are better. Use it
    // directly — no inversion.
    render: ({ benchmarks }) => {
      if (!benchmarks || benchmarks.sentiment_rank == null) return "—";
      return `${Math.round(benchmarks.sentiment_rank * 100)}th percentile`;
    },
    numeric: ({ benchmarks }) => {
      if (
        !benchmarks ||
      ...
      return benchmarks.sentiment_rank;
    },
  },
];
