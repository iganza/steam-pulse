...
def test_find_due_reviews_skips_when_delta_below_threshold(
    ...
    # 29d > 14d window + ~14d max smear → deterministically in window_due regardless
    # of appid hash bucket, while still inside the 30d safety floor — so the only
    # remaining reason to skip is the delta gate. Otherwise the test could pass
    # for the wrong reason (not yet window-due, or 30d safety overriding the gate).
    _set_review_crawled_at(catalog_repo, 8600, days_ago=29)
...
def test_find_due_reviews_returns_when_delta_meets_threshold(
    ...
    # 29d > 14d window + ~14d max smear → deterministically in window_due,
    # while staying inside the 30d safety floor.
    _set_review_crawled_at(catalog_repo, 8601, days_ago=29)
