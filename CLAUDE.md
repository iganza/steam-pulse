...
- **Optional metadata parameters** at the outer `analyze_game` boundary
  that may genuinely be absent (`temporal`, `metadata`, `steam_*`) —
  but even these should be required-with-explicit-None at the handler
  level, not defaulted to None in the signature, so callers are forced
  to think about whether they have the value.
